...
public class GetConfigurationByIdQueryHandler(BenchmarkingDbContext context)
    : IRequestHandler<GetConfigurationByIdQuery, ConfigurationHeader?>
{
    public async Task<ConfigurationHeader?> Handle(GetConfigurationByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            true => await context.Configurations.Where(c => c.Id == request.Id)
                .Include(c => c.ConfigurationItems)
                .Include(c => c.Constraints)
                .Select(c => new ConfigurationHeader(c))
                .FirstOrDefaultAsync(cancellationToken),
            false => await context.Configurations.Where(c => c.Id == request.Id)
                .Select(c => new ConfigurationHeader(c))
                .FirstOrDefaultAsync(cancellationToken)
        };
    }
